	...
	case $hacer in
		...
		2|[Hh]|[Hh][Aa][Rr][Dd][Ww][Aa][Rr][Ee])
			...
			if [ $LOCALE == es_ES.UTF-8 ]; then
				CPU_NAME=$(lscpu | grep "Nombre del modelo" | tr -s " " | cut -d" " -f4- | head -n 1)
			elif [ $LOCALE == C.UTF-8 ]; then 
				CPU_NAME=$(lscpu | grep "Model name" | tr -s " " | cut -d" " -f3- | head -n 1)
			fi

			CORES=$(lscpu | grep "Core(s) per socket:" | tr -s " " | cut -d" " -f4-)
			...
			VGA=$(lspci | grep "VGA" | cut -d" " -f5- | sed 's/ (rev.*)//')
			MOTHERBOARD=$(dmidecode --type system | grep "Product" | cut -d" " -f3-)

			if [ '$lspci | grep "VGA" | cut -d" " -f5- | wc -l' == 1 ]; then
				GRAPHICS=$(echo -e "Gráfica:\t${VGA}")
			else
				VGA1=$(echo "$VGA" | sed -n '1p')
				VGA2=$(echo "$VGA" | sed -n '2p')
				GRAPHICS=$(echo -e "Gráfica 1:\t${VGA1}" && echo -e "   Gráfica 2:\t${VGA2}")
			fi
